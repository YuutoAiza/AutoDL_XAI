...
def main(IMAGE_NUM):
    ...
    if IMAGE_NUM == MAX-1:
        ...
        xpath = "/Users/YULAB/Desktop/XAutoDL/data/.torch/cifar.python"
        train_data, valid_data, xshape, class_num = get_datasets("cifar10", xpath, -1)
        ...
        with torch.no_grad():
            ...
            for j, inputs in enumerate(valid_loader, 0):
                images, labels_ = inputs
                images = images.to(DEVICE)
                labels_ = labels_.to(DEVICE)

                outputs = arch(images)
                loss = criterion(outputs[1], labels_)
                test_loss += loss.item()

                predicted = outputs[1].max(1)[1]

                test_accuracy += (predicted == labels_).sum()

                running_test_loss += loss.item()
                if j % 2000 == 1999:
                    print('[%5d] loss: %.3f' % (j + 1, running_test_loss / 2000))
                    running_test_loss = 0.0
            
            avg_test_loss = test_loss / len(testloader.dataset)
            avg_test_accuracy = test_accuracy / len(testloader.dataset)
        
        test_loss_list.append(avg_test_loss)
        test_accuracy_list.append(avg_test_accuracy)
        print("test loss:", avg_test_loss, "test accuracy", avg_test_accuracy.item())

    target_layers = [arch.module.layers[54].conv_a.conv]
    ...
    if explanation_method == 0:
        ...
        saliency_map = ScoreCAM_extracor_2(input_tensor, arch, target_layers, TARGET_CLASS)
        directory =  "ScoreCAM"
